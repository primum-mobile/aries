...
class Fortune:
	...
	def __init__(self, typ, ascmc2, raequasc, pls, obl, placelat, abovehorizon, ayanamsha_offset=0.0):
		# ``ayanamsha_offset`` is the chart's ayanamsha value in degrees
		# (0.0 in tropical mode). Planet/ASC lons in ``pls``/``ascmc2``
		# are in the chart's chosen zodiac (sidereal when an ayanamsha
		# is selected), so the LoF formula self.fortune[LON] = ASC +
		# (Moon - Sun) lands in the same chosen zodiac — which is what
		# we want for display. But the RA/DECL conversion at the end
		# uses swe_cotrans, which is pure ecliptic→equatorial geometry
		# and needs a tropical input; we recover tropical by adding
		# ayanamsha_offset before the cotrans call.
		...
		self.fortune[Fortune.RA], self.fortune[Fortune.DECL], distprom = astrology.swe_cotrans(util.to_tropical_lon(self.fortune[Fortune.LON], ayanamsha_offset), 0.0, 1.0, -obl)
	...
	def calcMundaneProfPos(self, ascmc2, fort, placelat, obl, ayanamsha_offset=0.0):
		...
		if not okGa:
			...
			_okGa1, _okGd1, lon1 = self.iterate(fort, rao1, rdo, robl, rpoh, lon)
			rao2 = rao-math.radians(0.5)
			_okGa2, _okGd2, lon2 = self.iterate(fort, rao2, rdo, robl, rpoh, lon)
			lon = util.normalize(lon1 + (((lon2-lon1+180.0) % 360.0)-180.0)/2.0)
		elif not okGd:
			rdo1 = rdo+math.radians(0.5)
			_okGa1, _okGd1, lon1 = self.iterate(fort, rao, rdo1, robl, rpoh, lon)
			rdo2 = rdo-math.radians(0.5)
			_okGa2, _okGd2, lon2 = self.iterate(fort, rao, rdo2, robl, rpoh, lon)
			lon = util.normalize(lon1 + (((lon2-lon1+180.0) % 360.0)-180.0)/2.0)
