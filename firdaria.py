...
class Firdaria:
	...
	def __init__(self, by, bm, bd, opts, isdaily, isfirbonatti=None):
		self.by = by
		self.bm = bm
		self.bd = bd
		...
		self.options = opts
		if isfirbonatti is None:
			self.isfirbonatti = bool(getattr(self.options, 'isfirbonatti', True))
		else:
			self.isfirbonatti = bool(isfirbonatti)
