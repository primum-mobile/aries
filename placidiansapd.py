...
class PlacidianSAPD(placidiancommonpd.PlacidianCommonPD):
	...
	def calcCustomer2GlobalHouseCusps(self, mundane):
		...
		self._for_each_global_house_cusp_significator([
			lambda: self.toCustomer2(mundane, primdirs.PrimDir.CUSTOMERPD, primdirs.PrimDir.NONE, raprom, adprom, chart.Chart.CONJUNCTIO, 0.0, True)
		])
	...
	def calcCustomer2Customer2(self, mundane):
		...
		self.toCustomer2(mundane, primdirs.PrimDir.CUSTOMERPD, primdirs.PrimDir.NONE, raprom, adprom, chart.Chart.CONJUNCTIO)
				...
				for k in range(DEXTER+1):
					...
					if self.options.subzodiacal == primdirs.PrimDirs.SZPROMISSOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH:
						...
						if self.options.morin_excentric:
							lon, latprom = self.getMorinExcentric(p, plprom.data[planets.Planet.LONG], pllat, aspect)
...
#						raprom, declprom = util.getRaDecl(lon, latprom, self.chart.obl[0])
	...
	def calcAntiscia2Parallels(self):
		...
		if getattr(self.chart.antiscia, 'morin_antiscia', False):
			if getattr(self.chart.antiscia, 'plantiscia_secondary', None):
				self.calcAntiscia2ParallelsSub(self.chart.antiscia.plantiscia_secondary, primdirs.PrimDir.ANTISCION)
			if getattr(self.chart.antiscia, 'plcontraant_secondary', None):
				self.calcAntiscia2ParallelsSub(self.chart.antiscia.plcontraant_secondary, primdirs.PrimDir.CONTRAANT)
...
#					raprom, declprom = util.getRaDecl(lon, pllat, self.chart.obl[0])
	...
	def calcAntiscia2MLoF(self):
		...
		if getattr(self.chart.antiscia, 'morin_antiscia', False):
			if getattr(self.chart.antiscia, 'plantiscia_secondary', None):
				self.calcAntiscia2MLoFSub(self.chart.antiscia.plantiscia_secondary, primdirs.PrimDir.ANTISCION)
			if getattr(self.chart.antiscia, 'plcontraant_secondary', None):
				self.calcAntiscia2MLoFSub(self.chart.antiscia.plcontraant_secondary, primdirs.PrimDir.CONTRAANT)
			...
			if mundane or self.options.subzodiacal == primdirs.PrimDirs.SZSIGNIFICATOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH: #mundane or zod with sig's latitude
				if sigasp == chart.Chart.CONJUNCTIO:
					...
					if mundane:
						...
						if self.options.morin_excentric:
							lonsig, latsig = self.getMorinExcentric(sig, plsig.speculums[primdirs.PrimDirs.PLACSPECULUM][planets.Planet.LONG], latsig, aspect)
						elif self.options.bianchini:
	...
	def toCustomer2(self, mundane, idprom, idprom2, raprom, adprom, promasp, aspect = 0.0, calcsecmotion = False):
		...
		if mundane or self.options.subzodiacal == primdirs.PrimDirs.SZSIGNIFICATOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH: #mundane or zod with sig's latitude
			mdsig = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.MD]
			sasig = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.SA]
			...
			t, v, ra = self.getvars(point.abovehorizon, point.eastern)
		else:
			lonsig = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.LONG]
			ok, mdsig, sasig, abovehorizon, eastern = self.getZodMDSA(lonsig)
			if not ok:
	...
	def getZodMDSA(self, lon, lat=0.0):
		...
		key = (lon, lat)
		cached = self._zodmdsa_cache.get(key)
		...
		if math.fabs(val) > 1.0:
			result = (False, 0.0, 0.0, False, False)
			self._zodmdsa_cache[key] = result
			return result
		...
		result = (True, md, sa, abovehorizon, eastern)
		self._zodmdsa_cache[key] = result
		return result
...
#####################################Moon's SecMotion
	...
	def calcArcWithSMCustomer2(self, mundane, idprom, psidx, aspect, arc):
		...
		if mundane or self.options.subzodiacal == primdirs.PrimDirs.SZPROMISSOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH:
			latprom = 0.0
			if not mundane and self.options.morin_excentric:
				lon, latprom = self.getMorinExcentric(idprom, pllon, pllat, aspect)
			elif not mundane and self.options.bianchini:
				val = self.getBianchini(pllat, chart.Chart.Aspects[psidx])
				if math.fabs(val) > 1.0:
					return False, 0.0
				latprom = math.degrees(math.asin(val))
