...
def to_tropical_lon(lon, ayanamsha_offset):
	"""Recover a tropical longitude (angular distance from the vernal
	equinox) from a chart-stored ecliptic longitude.

	Aries stores planet, cusp, fixed-star, midpoint, LoF, ASC, MC, and
	customer-PD longitudes in the chart's chosen zodiac — sidereal when
	an ayanamsha is selected, tropical otherwise. ``ayanamsha_offset``
	is the actual offset in degrees (``Chart.ayanamsha_offset``); 0.0
	in tropical mode, ~24° in modern sidereal modes.

	The handful of operations that intrinsically need a tropical input
	all reach for this helper rather than re-implementing the recovery
	inline:

	- ``swe_cotrans`` calls (ecliptic→equatorial geometry needs angular
	  distance from the vernal equinox; the RA/decl result is intrinsic
	  to the actual point on the celestial sphere)
	- antiscia / dodecatemoria solstitial-axis mirrors (the solstitial
	  axis is a tropical phenomenon)
	- ``manazil.resolve_lon``'s contract (input must be tropical)
	- primary-direction term-arc coordinate transforms

	In tropical mode the helper is a no-op (``offset = 0``).
	"""
	return (lon + ayanamsha_offset) % 360.0
