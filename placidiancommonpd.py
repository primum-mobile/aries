	...
	def calc2HouseCusps(self, mundane):
		...
		for i in range(len(self.chart.planets.planets)):
			...
			if not mundane and self.options.subzodiacal != primdirs.PrimDirs.SZPROMISSOR and self.options.subzodiacal != primdirs.PrimDirs.SZBOTH:
				rapl, declpl, dist = astrology.swe_cotrans(self._lon_for_cotrans(pl.data[planets.Planet.LONG]), 0.0, 1.0, -self.chart.obl[0])
...
#						raprom, declprom = util.getRaDecl(lon, latprom, self.chart.obl[0])
	...
	def calcCustomer2HouseCusps(self, mundane):
		...
		lonpl = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.LONG]
		rapl = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.RA]
		dsa = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.SA]
		nsa = point.speculums[primdirs.PrimDirs.PLACSPECULUM][customerpd.CustomerPD.SA]
	...
	def calcAntiscia2HouseCusps(self, mundane):
		...
		if getattr(self.chart.antiscia, 'morin_antiscia', False):
			if getattr(self.chart.antiscia, 'plantiscia_secondary', None):
				self.calcAntiscia2HouseCuspsSub(mundane, self.chart.antiscia.plantiscia_secondary, primdirs.PrimDir.ANTISCION)
			if getattr(self.chart.antiscia, 'plcontraant_secondary', None):
				self.calcAntiscia2HouseCuspsSub(mundane, self.chart.antiscia.plcontraant_secondary, primdirs.PrimDir.CONTRAANT)
	...
	def calcHArcWithSM(self, mundane, idprom, h, arc, aspect, asp=0.0):
		...
		if not mundane:
			...
			if self.options.subzodiacal == primdirs.PrimDirs.SZPROMISSOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH:
				latprom = 0.0
				if self.options.morin_excentric:
					lonprom, latprom = self.getMorinExcentric(idprom, lonprom_naked, pllat, asp)
				elif self.options.bianchini:
