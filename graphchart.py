...
try:
	_FT = getattr(ImageFont, "FreeTypeFont", None)
	if _FT is not None and not hasattr(_FT, "getsize") and hasattr(_FT, "getbbox"):
		def _ft_getsize(self, text, *args, **kwargs):
			left, top, right, bottom = self.getbbox(text, *args, **kwargs)
			return (right - left, bottom - top)
		_FT.getsize = _ft_getsize
except Exception:
	pass

# Pillow 10+ removed ImageDraw.textsize(); legacy layout code uses it.
try:
	import PIL.ImageDraw as _PILImageDraw
	_ID = getattr(_PILImageDraw, "ImageDraw", None)
	if _ID is not None and not hasattr(_ID, "textsize") and hasattr(_ID, "textbbox"):
		def _id_textsize(self, text, font=None, *args, **kwargs):
			left, top, right, bottom = self.textbbox((0, 0), text, font=font, *args, **kwargs)
			return (right - left, bottom - top)
		_ID.textsize = _id_textsize
except Exception:
	pass
...
class GraphChart:
	...
	def __init__(self, chrt, size, opts, bw, planetaryday=True, chrt2 = None):
		...
		self.fntAspects = _sf(ImageFont.truetype(common.common.symbols, _fs(self.symbolSize/2)))
		...
		self.fntSmallText = _sf(ImageFont.truetype(common.common.abc, _fs(self.symbolSize/4)))
	...
	def drawChart(self):
		...
		if self.options.aspects:
			...
			if self.chart2 == None and self.options.showlof and self.options.showaspectstolof:
		...
		if self.chart2 == None and self.options.aspects and self.options.symbols:
		...
		for i in range(planets.Planets.PLANETS_NUM+1):
			...
			if i < planets.Planets.PLANETS_NUM:
				if chrt.planets.planets[i].data[planets.Planet.SPLON] <= 0.0:
					...
					self.draw.text((x-self.symbolSize/8, y-self.symbolSize/8), t, fill=clr, font=self.fntSmallText2 if t == 'SD' else self.fntSmallText)
