...
# ###########################################
	...
	def from_planet(cls, planet):
		...
		obj.latmin = None
		obj.latsec = None
		obj.southern = planet.data[cls.LAT] < 0.0
		obj.lon = planet.data[cls.LONG]
		obj.lat = planet.data[cls.LAT]
		obj.ra = planet.speculums[0][cls.RA]
		obj.decl = planet.speculums[0][cls.DECL]
		obj.speculums = [tuple(planet.speculums[0]), tuple(planet.speculums[1])]
		obj.eastern = getattr(planet, 'eastern', True)
		obj.abovehorizon = getattr(planet, 'abovehorizon', True)
		return obj
	...
	def computePlacidianSpeculum(self, placelat, ascmc2):
		...
		AZM, ELV = self._calc_horizontal(placelat, ascmc2, self.ra, self.decl)
		self.speculums.append((self.lon, self.lat, self.ra, self.decl, adlat, sa, md, self.hd, th, hod, pmp, adphi, phi, ao, AZM, ELV))
