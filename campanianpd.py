...
class CampanianPD(regiocampbasepd.RegioCampBasePD):
			...
			if mundane or self.options.subzodiacal == primdirs.PrimDirs.SZSIGNIFICATOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH: #mundane or zod with sig's latitude
				...
				if sigasp == chart.Chart.CONJUNCTIO:
					...
					if mundane:
						...
						val = self.sinlat*math.sin(math.radians(zdap))
						if math.fabs(val) > 1.0:
							continue
						...
						val = self.sinlat*math.sin(math.radians(cmpap))
						if math.fabs(val) > 1.0:
							continue
						...
						X = math.degrees(math.atan(self.coslat*math.tan(math.radians(cmpap))))
						...
						lonsig_base = plsig.speculums[primdirs.PrimDirs.REGIOSPECULUM][planets.Planet.LONG]
						lonsig = util.normalize(lonsig_base+aspect)
						latsig = plsig.speculums[primdirs.PrimDirs.REGIOSPECULUM][planets.Planet.LAT]

						ok, lonsig, latsig = self._zodiacal_aspect_position(sig, lonsig_base, latsig, sigasp, aspect)
						if not ok:
							continue
	...
	def toHCs(self, mundane, idprom, raprom, declprom, aspect, asp=0.0):
		...
		for i in range(1, houses.Houses.HOUSE_NUM+1):
			...
			dn = math.degrees(math.atan(math.tan(math.radians(ko))*self.coslat))
			if (dn < 0.0):
				dn += 180.0
		...
		for h in range(len(hcps)):
			...
			val = self.sinlat*math.sin(math.radians(zdsig))
			if math.fabs(val) > 1.0:
				continue
...
#####################################Moon's SecMotion
	...
	def calcHArcWithSM(self, mundane, idprom, h, hcps, arc, aspect, asp=0.0):
		...
		if not mundane:
			lonprom_base = lonprom
			lonprom = util.normalize(lonprom_base+asp)
			...
			if self.options.subzodiacal == primdirs.PrimDirs.SZPROMISSOR or self.options.subzodiacal == primdirs.PrimDirs.SZBOTH:
				ok, lonprom, latprom = self._zodiacal_aspect_position(idprom, lonprom_base, pllat, aspect, asp)
				if not ok:
					return False, 0.0
