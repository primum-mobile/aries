...
class PlaceDB:
	...
	def _resolve_user_path():
		user_path = os.path.join(app_paths.app_support_dir(), 'placedb.dat')
		legacy_path = os.path.join(app_paths.legacy_app_support_dir(), 'placedb.dat')
		app_paths.migrate_file_if_missing(legacy_path, user_path)
		return user_path

	@staticmethod
	def _resolve_legacy_user_path():
		return os.path.join(app_paths.legacy_app_support_dir(), 'placedb.dat')
	...
	def read(self):
		...
		if os.path.exists(user_path):
			try:
				...
				pass
		legacy_path = PlaceDB._resolve_legacy_user_path()
		if legacy_path != user_path and os.path.exists(legacy_path):
			try:
				self._load_from(legacy_path)
				return
			except Exception:
				pass
