...
class Planet:
	...
	Speculum indices (LONG..AODO / RMD..ELV) index into the speculum array
	returned by self.speculum or self.speculum2.  The speculum contains
	positional and house-placement data for one planet under a given house
	system.  Two separate index sets exist because Placidus and
	Regiomontanian/Campanian systems compute different intermediate values,
	but both share the first four ecliptic/equatorial slots (LONG-DECL).

	data[] indices (DIST..SPDIST) index into the raw SwissEph position
	array: [longitude, latitude, distance, speed_lon, speed_lat, speed_dist].

	dataEqu[] indices (RAEQU..SPDISTEQU) index into the equatorial position
	array returned by SwissEph with the SEFLG_EQUATORIAL flag.
	"""

	# ── Speculum indices: Common (all house systems) ──────────────
	LONG = 0       # Ecliptic longitude (0-360°)
	LAT = 1        # Ecliptic latitude (-90° to +90°)
	RA = 2         # Right Ascension (0-360°, equatorial)
	DECL = 3       # Declination (-90° to +90°, equatorial)

	# ── Speculum indices: Placidus house system ───────────────────
	ADLAT = 4      # Ascensional Difference under latitude
	SA = 5         # Semi-Arc (half diurnal/nocturnal arc)
	MD = 6         # Meridian Distance (angular distance to local meridian)
	HD = 7         # House Distance (distance within its house, 0-30°)
	TH = 8         # Temporary House (Placidian house number, 1-12)
	HOD = 9        # Hour Distance (hourly distance from an angle)
	PMP = 10       # Placidus Mundane Position (0-360° mundane longitude)
	ADPH = 11      # Ascensional Difference under Pole of the House
	POH = 12       # Pole of the House
	AODO = 13      # Ascensional Oblique Descension under Own pole
	PL_AZM = 14    # Astrological azimuth for Placidian display
	PL_ELV = 15    # Altitude/elevation for Placidian display

	# ── Speculum indices: Regiomontanian / Campanian ──────────────
	RMD = 4        # Meridian Distance (Regio, same slot as Placidus ADLAT)
	RHD = 5        # House Distance (Regio)
	ZD = 6         # Zenith Distance
	POLE = 7       # Pole of the house circle
	Q = 8          # Q parameter (intermediate for house position)
	W = 9          # W parameter (intermediate for house position)
	CMP = 10       # Campanus Mundane Position
	RMP = 11       # Regiomontanus Mundane Position
# ########################################
# Roberto change - V 7.1.0
	AZM = 12       # Azimuth (0-360°, horizon coordinate)
	ELV = 13       # Elevation / Altitude (horizon coordinate)
# ########################################

	# ── data[x] indices: raw SwissEph ecliptic position ───────────
	# data[0] = longitude, data[1] = latitude (same as LONG/LAT above)
	DIST = 2       # Distance (AU for planets, Earth radii for Moon)
	SPLON = 3      # Speed in longitude (°/day)
	SPLAT = 4      # Speed in latitude (°/day)
	SPDIST = 5     # Speed in distance (AU/day)

	# ── dataEqu[] indices: SwissEph equatorial position ───────────
	RAEQU = 0      # Right Ascension (equatorial)
	DECLEQU = 1    # Declination (equatorial)
	DISTEQU = 2    # Distance
	SPRAEQU = 3    # Speed in Right Ascension (°/day)
	SPDECLEQU = 4  # Speed in Declination (°/day)
	SPDISTEQU = 5  # Speed in distance
	...
	def computePlacidianSpeculum(self, lat, ascmc2):
		...
		AZM, ELV = self._calc_horizontal(lat, ascmc2, self.dataEqu[Planet.RAEQU], self.dataEqu[Planet.DECLEQU])
		self.speculums.append((self.data[Planet.LONG], self.data[Planet.LAT], self.dataEqu[Planet.RAEQU], self.dataEqu[Planet.DECLEQU], adlat, sa, md, self.hd, th, hod, pmp, adphi, phi, ao, AZM, ELV))
	...
	def _calc_horizontal(self, placelat, ascmc2, ra, decl):
		...
		if cos_elv != 0.0:
			val = (math.cos(math.radians(placelat))*math.sin(math.radians(decl))-math.sin(math.radians(placelat))*math.cos(math.radians(decl))*math.cos(math.radians(Han)))/cos_elv
			if math.fabs(val) <= 1.0:
...
#		print 'lon=%f' % lon
		...
		#Placidus
		AZM, ELV = self._calc_horizontal(placelat, ascmc2, self.dataEqu[Planet.RAEQU], self.dataEqu[Planet.DECLEQU])
		self.speculums[0] = (self.data[Planet.LONG], self.data[Planet.LAT], self.dataEqu[Planet.RAEQU], self.dataEqu[Planet.DECLEQU], pl.speculums[0][Planet.ADLAT], pl.speculums[0][Planet.SA], pl.speculums[0][Planet.MD], pl.speculums[0][Planet.HD], pl.speculums[0][Planet.TH], pl.speculums[0][Planet.HOD], pl.speculums[0][Planet.PMP], pl.speculums[0][Planet.ADPH], pl.speculums[0][Planet.POH], pl.speculums[0][Planet.AODO], AZM, ELV)
